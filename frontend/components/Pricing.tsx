...
export default function Pricing({ compact = false }: { compact?: boolean }) {
  ...
  return <section id="pricing" className="scroll-mt-24 bg-[#07080f] py-24 sm:py-32"><div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10"><div className="mx-auto max-w-3xl text-center"><p className="text-xs font-semibold uppercase tracking-[.18em] text-cyan-100">Pricing</p><h2 className="mt-4 text-4xl font-semibold tracking-[-.045em] text-white sm:text-5xl">Clear prices. Visible capacity. Free previews.</h2><p className="mt-5 leading-7 text-white/55">Free, Starter, and Pro use the same review-first workflow. The live Plan API supplies prices, daily credits, billing intervals, and configured limits.</p>{state === "fallback" && <p className="mt-4 inline-flex items-center gap-2 rounded-full border border-amber-300/20 bg-amber-300/[.06] px-3 py-2 text-xs text-amber-100"><CircleAlert size={14} /> Live plans are unavailable; standard plan values are shown.</p>}</div><div className="mt-12 grid gap-5 lg:grid-cols-3">{plans.map((plan) => <PlanCard key={plan.id} plan={plan} isAuthed={isAuthed} currentPlanId={currentPlanId} />)}</div>{!compact && <><CreditExplainer /><CompareTable plans={plans} /><PricingFAQ /><p className="mt-10 text-center text-xs text-white/35">Paid checkout uses Razorpay only when payment credentials are configured. Sandbox promo codes are never listed here.</p></>}</div></section>;
}
