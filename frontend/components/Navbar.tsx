...
function MobileDrawer({ open, onClose, active, onSelect, isAuthed }: { open: boolean; onClose: () => void; active: string; onSelect: (link: NavLink) => void; isAuthed: boolean }) {
  ...
  return <Sheet open={open} onOpenChange={(next) => !next && onClose()}><SheetContent side="right" className="w-full max-w-none px-4 pb-[calc(1.25rem+env(safe-area-inset-bottom))] pt-[calc(5.5rem+env(safe-area-inset-top))] sm:w-[min(92vw,26rem)]"><SheetTitle className="sr-only">Mobile navigation</SheetTitle><nav className="space-y-2" aria-label="Mobile navigation">{navGroups.map((group) => <div key={group.label} className="border-b border-white/10 py-2"><button type="button" aria-expanded={expanded === group.label} onClick={() => setExpanded(expanded === group.label ? null : group.label)} className="flex min-h-12 w-full items-center justify-between rounded-xl px-2 text-left text-lg font-semibold text-white">{group.label}<ChevronDown size={20} className={`transition-transform ${expanded === group.label ? "rotate-180" : ""}`} /></button>{expanded === group.label && <div className="pb-2">{group.links.map((link) => <NavButton key={link.label} link={link} active={active === link.section} onSelect={() => onSelect(link)} />)}</div>}</div>)}<NavButton link={primaryNav.pricing} active={active === "pricing"} onSelect={() => onSelect(primaryNav.pricing)} />{isAuthed ? <Link href="/dashboard" onClick={onClose} className="mt-3 flex min-h-12 items-center justify-center gap-2 rounded-full bg-gradient-to-r from-[#4f7cff] via-[#6d5ef7] to-[#8b5cf6] px-4 text-base font-semibold text-white"><LayoutDashboard size={20} />Open dashboard</Link> : <div className="grid gap-2 pt-3"><Link href="/login" onClick={onClose} className="flex min-h-12 items-center rounded-xl px-3 text-base font-semibold text-white/80">Log in</Link><Link href="/signup" onClick={onClose} className="flex min-h-12 items-center justify-center rounded-full border border-white/15 px-4 text-base font-semibold text-white">Sign up</Link></div>}</nav></SheetContent></Sheet>;
}
